...
PRODUCTION
  Real missions             N / 100
  Successful                     —
  Completed with exception       —   (waiver on mission.terminalOutcome)
  Failed                         —
  In progress                    —
THROUGHPUT
  Missions / day                 —
...
CRM note: PASS
Draft message: PASS
Task create: WAIVED — GHL HTTP 422
Approvals: 3
Executions: 13
Execution cost: 30 units
Human intervention: recorded
Workflow defect found: create-vs-update routing
Workflow defect fixed: yes
External integration defect: crm.task.create / GHL
```

Persist via `PATCH /v1/missions/:missionId` with
`metadata.terminalOutcome` (status `completed_with_exception`).
Core `Mission.status` becomes `completed`; Console/scoreboard read the
exception from metadata so the waiver is not silent.

**Cohort signal already useful:** OL-001 **1 / 100**. M001 surfaced entity-state
routing defect (fixed), GHL task compatibility defect, 3 human interventions,
~30 cost units.
